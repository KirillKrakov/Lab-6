...
public class FileManager {
    ...
    public TreeSet<LabWork> fromXMLtoCollection() {
        try {
            ...
            ServerApp.logger.info("Коллекция успешно загружена.");
            ...
            ServerApp.logger.error("Произошла ошибка при чтении xml-файла в коллекцию!");
            ex.printStackTrace();
        } catch (FileNotFoundException ex) {
            Outputer.printerror("Файл с таким именем не найден!");
            ServerApp.logger.warn("Файл с таким именем не найден!");
        }
        return new TreeSet<>();
    }
}
